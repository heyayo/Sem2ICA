...
Mesh* MeshBuilder::GenerateLine(const std::string& meshName, Color color, float length)
{
	...
	//x-axis
	v.pos.Set(0, -length / 2.f, 0);	v.color = color;	vertex_buffer_data.push_back(v);
	v.pos.Set(0, length / 2.f, 0);	v.color = color;	vertex_buffer_data.push_back(v);
...
Mesh* MeshBuilder::GenerateQuad(const std::string &meshName, Color color, float length)
{
	...
	v.pos.Set(0.5f * length, -0.5f * length, 0.f); v.texCoord = {1,0};	vertex_buffer_data.push_back(v); //v3
	v.pos.Set(0.5f * length, 0.5f * length, 0.f); v.texCoord = {1,1};	vertex_buffer_data.push_back(v); //v0
	v.pos.Set(-0.5f * length, 0.5f * length, 0.f); v.texCoord = {0,1};	vertex_buffer_data.push_back(v); //v1
	v.pos.Set(-0.5f * length, -0.5f * length, 0.f); v.texCoord = {0,0};	vertex_buffer_data.push_back(v); //v2
